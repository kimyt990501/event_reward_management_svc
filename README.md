...
- 모든 api 응답을 응답 인터셉터로 감싸서 같은 형식으로 응답 받게 설정
- 성공 응답 인터셉터:
```json
{
  "code": "SUCCESS",
...
- 위의 이벤트들은 따로 관리자가 보상 요청을 확인하여 승인/거절할 필요 없이 내부 로직으로 자동 승인/거절하게 설계하였습니다.
  - 3일 연속 출석 이벤트 및 보상, 3명 초대 이벤트 및 보상 데이터는 초기 docker compose 실행 시 자동으로 추가되게 하였습니다.
...
docker info
```
- 위의 코드 결과로 도커 정보가 나온다면 도커가 실행 중이므로 바로 B 단계로 이동 
- 도커 정보가 출력되지 않는다면 docker desktop 실행한 다음 다시 확인 후 B 단계로 이동
...
- 물론 소규모 과제였지만 인증 파트부터 기능 구현, 데이터베이스 설계까지 전부 해본건 이번이 처음이었습니다. 그래서 그런지 중간중간 어디까지 했고 어느 부분을 할 차례인지 헷갈릴 때도 있었지만 기능 체크리스트를 작성하며 해당 문제는 잘 해결됐고 여러모로 뜻깊은 과제였던것 같습니다.!
